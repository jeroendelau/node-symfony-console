...
xdescribe('#Application - Error Output', () => {
  it('testRenderExceptionEscapesLines', async () => {
    let application = new Application();
    application.setAutoExit(false);
    putenv('COLUMNS=22');
    application.register('foo').setCode(function () {
      throw new Error('dont break here <info>!</info>');
    });
    let tester = new ApplicationTester(application);

    await tester.run({'command': 'foo'}, {'decorated': false});
    assert.deepEqual(tester.getDisplay(true), fixtureContent('application_renderexception_escapeslines.txt'), '.Error() escapes lines containing formatting');
    putenv('COLUMNS=120');
  });

  it('testRenderExceptionLineBreaks', async () => {
    let application = sinon.createStubInstance(Application);
    application.getTerminalWidth.returns(120);
    application.setAutoExit.restore();
    application.register.restore();
    application.setAutoExit(false);
    ...
    await tester.run({'command': 'foo'}, {'decorated': false});
    assert.deepEqual(fixtureContent('application_renderexception_linebreaks.txt'), tester.getDisplay(true), '.Error() keep multiple line breaks');
  });

  it('testRenderAnonymousException', async () => {
    let application = new Application();
    application.setAutoExit(false);
    application.register('foo').setCode(function () {
    ...
    await tester.run({'command': 'foo'}, {'decorated': false});
    assert.deepEqual('[Error@anonymous]', tester.getDisplay(true));

    application = new Application();
    application.setAutoExit(false);
    application.register('foo').setCode(function () {
      // throw new Error(sprintf('Dummy type "%s" is invalid + ', get_class(new class() {})));
    });
    tester = new ApplicationTester(application);

    await tester.run({'command': 'foo'}, {'decorated': false});
    assert.isStringContainsString('Dummy type "@anonymous" is invalid + ', tester.getDisplay(true));
  });

  it('testRenderExceptionStackTraceContainsRootException', async () => {
    let application = new Application();
    application.setAutoExit(false);
    application.register('foo').setCode(function () {
      // throw new class('') extends \Error {};
    ...
    await tester.run({'command': 'foo'}, {'decorated': false});
    assert.isStringContainsString('[Error@anonymous]', tester.getDisplay(true));
